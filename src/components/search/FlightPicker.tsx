...
import { useSearchStore, type FlightDirection } from '../../stores/useSearchStore';
import { useTripStore } from '../../stores/useTripStore';
import { useUIStore } from '../../stores/useUIStore';

/**
 * 1단계: 왕복 항공편 선택 — 고객의 실제 예약 순서를 따라 항공을 가장 먼저 고른다.
 * 가는 편 / 오는 편 각각 최저가가 기본 선택되며, 사용자가 언제든 바꿀 수 있다.
 */
export function FlightPicker({ flights }: { flights: FlightOffer[] }) {
  ...
  return (
    <div className="space-y-4">
      <FlightGroup direction="outbound" label="가는 편" icon={<PlaneTakeoff size={14} />} flights={outbound} />
      <FlightGroup direction="return" label="오는 편 (귀국)" icon={<PlaneLanding size={14} />} flights={returns} />
    </div>
  );
}
...
function FlightGroup({
  ...
  const selectedId = useSearchStore((s) =>
    direction === 'outbound' ? s.selectedOutboundId : s.selectedReturnId,
  );
  ...
  if (flights.length === 0) {
    return (
      <div>
        <h4 className="mb-1.5 flex items-center gap-1.5 text-sm font-semibold text-ink-700">{icon} {label}</h4>
        <p className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800" role="alert">
          {label} 항공편을 찾지 못했어요. 대화에서 재검색을 요청해주세요.
        </p>
      </div>
    );
  }
  ...
  const choose = (f: FlightOffer) => {
    setSelectedFlight(direction, f.supplierOfferId);
    logEvent('OFFER_SELECTED', `${label} 항공편 선택: ${f.airline} ${f.flightNumber}`);
    pushToast('success', `${label} — ${f.airline} ${f.flightNumber} 선택됨`);
  };

  return (
    <div>
      <h4 className="mb-1.5 flex items-center gap-1.5 text-sm font-semibold text-ink-700">
        {icon} {label}
      </h4>
      <div className="space-y-2" role="radiogroup" aria-label={`${label} 항공편 선택`}>
        {flights.map((f) => {
          const selected = f.supplierOfferId === selectedId;
          return (
            <button
