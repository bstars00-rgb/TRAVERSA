  ...
  return (
    <div className="border-t border-ink-100 bg-ink-50">
      <button
        ...
        aria-expanded={open}
        className="flex w-full items-center gap-2 px-4 py-2 text-xs font-medium text-ink-500 hover:text-ink-700 cursor-pointer"
      >
        <Bot size={13} />
        전문 에이전트 8명
      ...
      {open && (
        <ul className="grid grid-cols-2 gap-1 px-3 pb-3">
          {agents.map((a) => (
            <li key={a.id} className="flex items-center gap-1.5 rounded-md bg-white px-2 py-1.5 text-xs">
